...
int main(int argc, char** argv) {
    ...
    int real_b=0;
    pthread_mutex_t mxreal_b = PTHREAD_MUTEX_INITIALIZER;
	srand(time(NULL));
...
void* throwing_func(void* voidArgs) {
    ...
    pthread_mutex_lock(args->pmxach);
		(*args->ach)+=1;
		pthread_mutex_unlock(args->pmxach);   
	while (1) {
