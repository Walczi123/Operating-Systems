...
int main(int argc, char** argv) {
	...
	argsEstimation_t* estimations = (argsEstimation_t*) \
        malloc(sizeof(argsEstimation_t) * threadCount);
	...
	for (int i = 0; i < threadCount; i++) {
		int err = pthread_create(&(estimations[i].tid), NULL, \
                        pi_estimation, &estimations[i]);
