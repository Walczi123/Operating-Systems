...
void prepare_request(char **argv, char data[10]){
    pid_t pid = getpid();
    char buff[10];
    printf("my pid: %d\n", pid);
    sprintf(buff, "%d", pid);
...
void print_answer(int16_t data[1]){
    printf("sum: %d\n", ntohs(data[0]));
...
int main(int argc, char** argv) {
	...
	if(bulk_write(fd,(char *)data,sizeof(char[10]))<0) ERR("write:");
    if(bulk_read(fd,(char *)dataRec,sizeof(int16_t[1]))<0) ERR("read:");
    print_answer(dataRec);
